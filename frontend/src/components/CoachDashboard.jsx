...
const ClientListContent = () => {
  ...
  useEffect(() => {
    const fetchClients = async () => {
      const coachId = localStorage.getItem("userId"); // Retrieve coachId from localStorage
    ...
    <div className="space-y-6">
      <Card>
        ...
        {loading ? (
          ...
          <p className="text-center text-gray-400">No clients assigned.</p>
        ) : (
          <div className="space-y-4">
            {clients.map((client) => (
              <ClientOverviewCard key={client.memberId} client={client} />
            ))}
          </div>
        )}
      </Card>
